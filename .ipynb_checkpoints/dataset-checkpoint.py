...
from config import generator, seed
...
class Baseset():
    def __init__(self, args, config, source, sample):
        ...
        self.labels_raw = self.adata.obs[self.cell_type]
...
class Dataset():
    def __init__(self, args, config):
        ...
        for source, sample in zip(args.sources, args.samples, strict=True):
            baseset = Baseset(args, config, source, sample)
            cell_ids.extend(baseset.cell_ids)
            images.extend(baseset.images)
            expressions.extend(baseset.expressions)
            labels_raw.extend(baseset.labels_raw)
            labels.extend(baseset.labels)
            for gene in baseset.genes:
                if gene not in genes:
                    genes.append(gene)
            stem_file += f"_{source}_{sample}"
            columns.extend(baseset.columns)
        ...
        class_indices = {c: np.where(np.array(labels_raw) == c)[0].tolist() for c in self.classes}
        print({c:len(indices) for c, indices in class_indices.items()})
        ...
        for c, indices in class_indices.items():
            ...
            expanded = indices * repeat + rng.choice(indices, remain, replace=False).tolist()
            for index in expanded:
                angle = int(rng.choice(angles))
                flip = str(rng.choice(flips))
                self.dataset.append((index, angle, flip))
                class_counts[labels_raw[index]] += 1
    ...
    def __getitem__(self, i):
        index, angle, flip = self.dataset[i]
