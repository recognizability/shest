...
```
* The `file_path` of the WSI file and the `path` of the execution directory must be specified as command-line arguments. Model weight `*.pth` files must be located under `models` within the `path`, and result files are generated in the `he` directory under the `path`.
