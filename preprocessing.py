...
def filter_cells(adata_obs: pd.DataFrame) -> tuple[pd.Index,int]:
    ...
    cell_boundaries = sdata.shapes['cell_boundaries'].loc[cell_id,'geometry']
    rectangle = cell_boundaries.minimum_rotated_rectangle.exterior.coords[:-1]
    edges = np.linalg.norm(np.diff(rectangle,axis=0),axis=1)
    size = round(max(edges)/PIXEL_SIZE)
